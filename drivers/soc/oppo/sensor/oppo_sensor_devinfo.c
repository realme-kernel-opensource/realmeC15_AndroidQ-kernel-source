...
//#ifdef VENDOR_EDIT
//Licheng@bsp.sensor 2020/07/09 add for sensor can use lcd info
	if(strstr(saved_command_line, "mdss_dsi_ili9881h_boe_90hz_video")){
		chip->als_lcd_type = BOE;
		pr_info("als-lcd-match rc is BOE\n");
	}else if(strstr(saved_command_line,"mdss_dsi_ili9881h_hlt_90hz_video")){
		chip->als_lcd_type = HLT;
		pr_info("als-lcd-match rc is HLT\n");
	}else{
		chip->als_lcd_type = BOE;
		pr_info("als-lcd-match rc first is BOE\n");
	}
//#endif

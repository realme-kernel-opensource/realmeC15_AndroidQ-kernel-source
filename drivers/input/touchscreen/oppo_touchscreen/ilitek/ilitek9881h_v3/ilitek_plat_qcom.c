                                  ...
                                  u8 read_type)
{
    ...
    ipio_err("Read debug data start \n");
    ...
    ipio_err(" wait mp irq status 100 \n");
    ...
    ipio_err("Read debug data end  \n");

    ipio_err("switch demo mode start \n");
    /* change to demo mode */
    tp_mode = P5_X_FW_DEMO_MODE;
    ilitek_tddi_switch_mode(&tp_mode);
    mutex_unlock(&idev->touch_mutex);
    ipio_kfree((void **)&buf);
    ipio_err("switch demo mode end \n");
    return 0;
}

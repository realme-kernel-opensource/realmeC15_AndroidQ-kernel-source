...
out:
	...
	if (5 == truly_version)
		sprintf(Ctp_name, "TRULY_25,ILI9882N,FW:0x%x\n", buf[3]);
	else if (6 == truly_version)
		sprintf(Ctp_name, "TRULY_26,ILI9882N,FW:0x%x\n", buf[3]);
	else if (2 == truly_version)
		sprintf(Ctp_name, "CDOT,ILI9882N,FW:0x%x\n", buf[3]);
	else
		ILI_ERR("Not fund touchscreen !");

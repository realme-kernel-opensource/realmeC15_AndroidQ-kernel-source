...
info_retry:
	...
	if (g_tp_dev_vendor == 2)
		sprintf(Ctp_name, "INX,NT36525B,fw_ver = 0x%x,fw_sub_ver = 0x%x\n", buf[1], buf[14]);
	else if (g_tp_dev_vendor == 0)
		sprintf(Ctp_name, "BOE_B3,NT36525B,fw_ver = 0x%x,fw_sub_ver = 0x%x\n", buf[1], buf[14]);
	else if (g_tp_dev_vendor == 3)
		sprintf(Ctp_name, "BOE_B8,NT36525B,fw_ver = 0x%x,fw_sub_ver = 0x%x\n", buf[1], buf[14]);
	else
        ...
        if (panel_data->manufacture_info.version) {
            if (panel_data->vid_len == 0) {
                ...
                if (ver_len <= 8) {
                    ...
                    strlcpy(&panel_data->manufacture_info.version[16], dev_version, MAX_DEVICE_VERSION_LENGTH - 16);
                }
            } else {

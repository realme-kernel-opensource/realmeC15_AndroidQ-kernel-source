	...
	struct etspi_data *data)
{
	...
	data->vreg = regulator_get(&data->spi->dev,"vdd_fp");
	if (!data->vreg) {
	...
	if (regulator_count_voltages(data->vreg) > 0) {
		ret = regulator_set_voltage(data->vreg, 3300000,3300000);
		if (ret){
			pr_err("Unable to set voltage on vdd_fp");
			goto dt_exit;
		}
	}
	ret = regulator_enable(data->vreg);
	if (ret) {

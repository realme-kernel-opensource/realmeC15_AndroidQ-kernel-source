...
static int fp_gpio_parse_parent_dts(struct fp_data *fp_data)
{
    ...
    for (fp_id_index = 0; fp_id_index < fp_data->fp_id_amount; fp_id_index++) {
        ...
        dev_info(fp_data->dev, "gpio_index: %d,fp_id: %d, this is chipone ic\n", fp_data->gpio_index[fp_id_index], fp_data->fp_id[fp_id_index]);

        if (fp_data->fp_id[fp_id_index] == 0) {
            dev_info(fp_data->dev, "the value of compatible gpio is 0, and need to recompatibel\n");
            fp_data->pinctrl = devm_pinctrl_get(dev);
            if (IS_ERR_OR_NULL(fp_data->pinctrl)) {
                dev_err(fp_data->dev,"Failed to get pinctrl, please check dts");
                ret = PTR_ERR(fp_data->pinctrl);
                goto exit;
	        }

            fp_data->input_active = pinctrl_lookup_state(fp_data->pinctrl, "input_high");
            if (IS_ERR_OR_NULL(fp_data->input_active)) {
                dev_err(fp_data->dev,"fp_data->input_active not found");
                ret = PTR_ERR(fp_data->input_active);
                goto err_pinctrl_lookup;
            }

            ret = pinctrl_select_state(fp_data->pinctrl, fp_data->input_active);
		    if (ret < 0) {
			      dev_err(fp_data->dev,"pinctrl_select_state error:%d", ret);
				  goto err_pinctrl_lookup;
		    }

            fp_data->fp_id[fp_id_index] = gpio_get_value(fp_data->gpio_index[fp_id_index]);
            dev_info(fp_data->dev, "the value while pull up is fp_id: %d\n", fp_data->fp_id[fp_id_index]);
            if ( fp_data->fp_id[fp_id_index] == 0)
                dev_info(fp_data->dev, "this is silead ic\n");
            else {
                fp_data->fp_id[fp_id_index] = 2;
                dev_info(fp_data->dev, "the compatible gpio is NC, this is eigs ic\n");
            }
        }
    }
    return ret;
err_pinctrl_lookup:
    devm_pinctrl_put(fp_data->pinctrl);
exit:
    return ret;
}
